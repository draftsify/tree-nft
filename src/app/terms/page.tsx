...
const CLAUSES: Clause[] = [
  ...
  },
  {
    ...
    body: [
      `${MINT.split[0].pct}% of gross mint revenue leaves the buyer for a fixed reforestation address inside the minting transaction. The contract enforces that step and the address cannot be changed after deployment.`,
      `That balance does not reach ${PARTNER.name} directly. The mint settles in our own token on Robinhood Chain, and a charity donation address accepts assets it can realise, so the reserve is swapped to ETH, bridged to Ethereum mainnet and only then donated. The route is published step by step on the impact page, with a transaction hash against each step and the amount actually received rather than a quoted value.`,
      "Between the reserve and the charity, those steps are carried out by us. They are not enforced by code and you are relying on us to complete them. What we offer instead of a promise is an audit trail: the reserve address is fixed and public, its balance is readable by anyone, and a batch stays open on the ledger until its final donation hash exists. The bridge alone takes about a week.",
      "The donations are anonymous. Anyone may send to a public donation address, no agreement is required for a donation to be valid, and none exists.",
